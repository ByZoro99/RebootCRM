# CRM — Contexto y Memoria del Proyecto

> Este archivo mantiene el contexto, las decisiones y el estado del proyecto CRM.
> Se actualiza a medida que avanzamos. El historial detallado de cambios está en `CHANGELOG.md`.

---

## 1. Resumen del proyecto

- **Nombre:** CRM (Reboot)
- **Ubicación:** `c:\xampp\htdocs\CRM`
- **Entorno:** XAMPP (Apache + MySQL + PHP) en Windows
- **Estado:** 🟡 Inicialización — definiendo objetivos y stack
- **Repositorio git:** local (rama `main`)

## 2. Objetivo

> _(Pendiente de definir con detalle)_
> ¿Qué problema resuelve este CRM? ¿Quiénes son los usuarios? ¿Qué gestiona
> (clientes, ventas, soporte, reventas...)?

## 3. Stack técnico

| Capa            | Tecnología        | Notas                          |
|-----------------|-------------------|--------------------------------|
| Backend         | PHP (por definir) | ¿Framework o PHP plano?        |
| Base de datos   | MySQL (XAMPP)     | Nombre de BD por definir       |
| Frontend        | Por definir       | HTML/CSS/JS, Bootstrap, etc.   |
| Autenticación   | Por definir       | Roles/permisos                 |

## 4. Estructura de carpetas

```
CRM/
├── CONTEXT.md      ← este archivo (contexto y memoria)
├── CHANGELOG.md    ← historial de versiones/cambios
└── .gitignore
```

## 5. Decisiones clave

| Fecha       | Decisión                          | Motivo |
|-------------|-----------------------------------|--------|
| 2026-06-13  | Inicializar git + archivo de contexto | Control de versiones y memoria del proyecto |

## 6. Tareas / Roadmap

- [ ] Definir objetivo y alcance del CRM
- [ ] Elegir stack (framework PHP, frontend)
- [ ] Diseñar el modelo de datos (entidades principales)
- [ ] Estructura base del proyecto

## 7. Notas y pendientes

> _(Apuntes sueltos, dudas, ideas)_

---

_Última actualización: 2026-06-13_
